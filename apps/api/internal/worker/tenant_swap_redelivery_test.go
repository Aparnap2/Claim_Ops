...
// APA-28 RED: tenant-swapped redelivery must fail closed.
//
// Seam choice: worker.Processor.Handle owns the durable work identity
// (in-memory processed-set keyed by document ID + stable investigation ID
// inputs + store writes keyed by event tenant). The transport-/app-layer
// attrs-vs-bytes binding is covered from the app package
// (internal/app/tenant_swap_redelivery_test.go); THIS file proves the
// processor seam: once document X is durably decided under tenant A, a
// redelivery of the same IDs under tenant B must be an explicit
// TERMINAL rejection — never SUCCESS, never DUPLICATE (which would adopt
// tenant A's execution, envelope included), with no new workflow launch
// and no store writes under either tenant.
//
...
func TestTenantSwapRedelivery_FailClosed(t *testing.T) {
	...
	// Adversarial redelivery: SAME durable work identity (same claim and
	// document IDs) but tenant B, delivered the way the pull loop would
	// (attrs tenant B on ctx, bytes tenant B in the event).
